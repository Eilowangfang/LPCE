...
bool LPCECheck     = false;   //must turn on if re-optimization is on
bool LPCERerun     = false;  //indicate current execution is rerun
bool LPCEEarlyStop = false; //indicate the init execution is early stopped

bool testPurpose   = false;
struct card_Runlog card_runLog[NODE_NUM_MAX] ={{0, 0, 0, 0, 0, 0, false, false}};
struct baseRel_Estimate_Card baseRel_EstimateCard[BASEREL_NUM_MAX] = {{0,0}};
