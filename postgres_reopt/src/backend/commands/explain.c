...
//        elog(INFO, "     ------> LPCE EXPLAIN ANALYZE ReQuery\n %s \n", ReQuery);
//
//
//         FILE *fp = NULL;
...
//
//        elog(INFO, "     ------> LPCE ==========================================\n");
//        elog(INFO, "     ------> LPCE READY TO EXECUTE RE-OPTIMIZED QUERY");
//        elog(INFO, "     ------> LPCE ==========================================\n");
//     }
    ...
    for(int k = 0; k < NODE_NUM_MAX; k++)
        elog(INFO, "     ------>------>------> LPCE node id %ld    %ld    %lf    %ld", k,   card_runLog[k].done_subplan, card_runLog[k].node_card, card_runLog[k].done_work);

	//LPCE
    if(LPCECheck && LPCERerun) {
        LPCEEarlyStop = false;
        is_InitRun = false;
        elog(INFO, "     LPCE ################################################ REPORT QUERY START\n");
        ExplainQuery_Rerun(pstate, stmt,
                           params, dest);
        elog(INFO, "     LPCE ################################################ REPORT QUERY END\n");
    }

    //clear run log data structure
    for(int i = 0; i < NODE_NUM_MAX; i++) {
        card_runLog[i].node_card     = 0;
        card_runLog[i].estimate_card = 0;
			...
			ExplainState *es)
{
	...
	if (planstate->instrument)
        ...
        //elog(INFO, "     ------> LPCE plan node %ld estimate tuples %0f", plan->plan_node_id, planstate->instrument->ntuples);
...
//    //get plan from checkpoint
    ...
    //elog(INFO, "     ------> LPCE tableReoptQuery\n %s \n", tableReoptQuery);
    //elog(INFO, "     ------> LPCE selectReoptQuery\n %s \n", selectReoptQuery);
    //elog(INFO, "     ------> LPCE joinReoptQuery\n %s \n", joinReoptQuery);
