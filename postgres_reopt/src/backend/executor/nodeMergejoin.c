...
//LPCE
double tmpCounterOuter = 0;
double tmpCounterInner = 0;
/*
	...
	for (;;)
	{
		...
		switch (node->mj_JoinState)
		{
			...
			case EXEC_MJ_INITIALIZE_OUTER:
                ...
                elog(INFO, "     ------> LPCE ExecMergeJoin EXEC_MJ_INITIALIZE_OUTER %ld", tmpCounter);
				break;
			...
			case EXEC_MJ_INITIALIZE_INNER:
                ...
                * */
				if(LPCECheck && LPCEEarlyStop){
                    //elog(INFO, "     ------> LPCE ExecMergeJoin UPPPDATE child status");
                    int mergeJoin_node_id = node->js.ps.plan->plan_node_id;
                ...
                elog(INFO, "     ------> LPCE ExecMergeJoin EXEC_MJ_INITIALIZE_INNER %ld", tmpCounter);
			...
			case EXEC_MJ_NEXTOUTER:
				...
				switch (MJEvalOuterValues(node))
				{
					...
					case MJEVAL_ENDOFJOIN:
						...
						/* Otherwise we're done. */
                        elog(INFO, "     ==================>>> LPCE ExecMergeJoin DONE DONE DONE %lf %lf", tmpCounterInner, tmpCounterOuter);
                        int outerSortNode_id  = card_runLog[node->js.ps.plan->plan_node_id].outer_node_id;
                        int innerSortNode_id  = card_runLog[node->js.ps.plan->plan_node_id].inner_node_id;
                        if(card_runLog[outerSortNode_id].node_card < 1){
                            card_runLog[outerSortNode_id].node_card = tmpCounterOuter;
                            card_runLog[outerSortNode_id].done_subplan = true;
                        }
                        if(card_runLog[innerSortNode_id].node_card < 1){
                            card_runLog[innerSortNode_id].node_card = tmpCounterInner;
                            card_runLog[innerSortNode_id].done_subplan = true;
                        }
                        tmpCounterInner = 0;
                        tmpCounterOuter = 0;
						return NULL;
			...
			case EXEC_MJ_SKIPOUTER_ADVANCE:
				...
				node->mj_MatchedOuter = false;
                tmpCounterOuter++;
                //elog(INFO, "     ==================>>> LPCE ExecMergeJoin EXEC_MJ_SKIPOUTER_ADVANCE %lf", tmpCounterOuter);
			...
			case EXEC_MJ_ENDOUTER:
				...
				{
					MJ_printf("ExecMergeJoin: end of inner subplan\n");
                    elog(INFO, "     ==================>>> LPCE ExecMergeJoin inner subplan %lf", tmpCounterOuter);
					return NULL;
				}
			...
			case EXEC_MJ_ENDINNER:
				...
				{
					MJ_printf("ExecMergeJoin: end of outer subplan\n");
                    elog(INFO, "     ==================>>> LPCE ExecMergeJoin outer subplan %lf", tmpCounterOuter);
					return NULL;
				}
...
ExecInitMergeJoin(MergeJoin *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> LPCE ExecInitMergeJoin at node %ld estimate tuples %.0f", mergestate->js.ps.plan->plan_node_id, mergestate->js.ps.plan->plan_rows);
    //elog(INFO, "     ------> LPCE ExecInitMergeJoin###################inner %ld  outer %ld", mergestate->js.ps.plan->inner_relids, mergestate->js.ps.plan->outer_relids);
	/*
...
ExecEndMergeJoin(MergeJoinState *node)
{
    ...
    elog(INFO, "     ------>------> LPCE ExecEndMergeJoin ");
	/*
