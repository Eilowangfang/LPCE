...
#include "executor/nodeMaterial.h" //LPCE
...
MultiExecPrivateHash(HashState *node)
{
	...
	//LPCE
	int tupleCount = 0;
    //elog(INFO, "     ------>------>------> LPCE MultiExecPrivateHash enter once");
	...
	for (;;)
	{
		...
		//   print_slot(slot);
        //LPCE feed tuple into materialization
...
MaterialState * ExecInitMaterialAtHashNode(){
    ...
    elog(INFO, "     ------>------>------> LPCE ExecInitMaterialAtHashNode");
    return matstate;
}
...
ExecInitHash(Hash *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> LPCE ExecInitHash at node %ld estimate tuples %.0f", hashstate->ps.plan->plan_node_id, hashstate->ps.plan->plan_rows);
    //elog(INFO, "     ------> LPCE ExecInitHash###################inner %ld  outer %ld", hashstate->ps.plan->inner_relids, hashstate->ps.plan->outer_relids);
...
#endif
    elog(INFO, "     ------>------>------> LPCE ExecHashJoinImpl -> ExecHashTableCreate");
	/*
					...
					uint32 hashvalue)
{
	...
	if (batchno == hashtable->curbatch)
	{
        ...
        //elog(INFO, "     ------>------>------> LPCE ExecHashTableInsert %ld", tuple->t_len);
