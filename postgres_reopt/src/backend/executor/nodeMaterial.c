...
ExecMaterial(PlanState *pstate)
{
	...
	if (eof_tuplestore && !node->eof_underlying)
	{
		...
		{
		    //LPCE
		    //Here is ok to use a global counter,
		    // since two materialization nodes must be blocked processing.
            double actual_tuples =  temp_counter - 1;
		    temp_counter = 0;
            elog(INFO, "     +++++++>++++++> +++++++>++++++> DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG");
            elog(INFO, "     +++++++>++++++> Material node_id: %d %lf", outerPlanState(node)->plan->plan_node_id, actual_tuples);
            elog(INFO, "     +++++++>++++++> +++++++>++++++> DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG");

            if(actual_tuples > 0) {
                //This means the tuple collected from outer plan is >= 1.
                //The outer plan works successfully.
                //Then we can update the outer plan info.
                //update info for outer node
                int outer_node_id = outerPlanState(node)->plan->plan_node_id;
                card_runLog[outer_node_id].node_card = actual_tuples;
                card_runLog[outer_node_id].done_subplan = true;
            }


            if(LPCECheck && !LPCERerun){
                if(actual_tuples < 1.0)
                    actual_tuples = 1.0;
                if(node->ss.ps.plan->plan_rows < 1.0)
                    node->ss.ps.plan->plan_rows = 1.0;

                if(node->ss.ps.plan->plan_node_id >= 4 &&
                        ((actual_tuples / node->ss.ps.plan->plan_rows) > 50.0 || (node->ss.ps.plan->plan_rows / actual_tuples) > 50.0)){
                    LPCERerun   =  true;
                    earlyStop   =  true;
                    LPCEEarlyStop = true;
                    elog(INFO, "     +++++++>++++++> LPCE ExecMaterial Trigger Re-optimization Point: %d %lf", node->ss.ps.plan->plan_node_id);
...
ExecInitMaterial(Material *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> LPCE ExecInitMaterial at node %ld estimate tuples %.0f", matstate->ss.ps.plan->plan_node_id, matstate->ss.ps.plan->plan_rows);
    //elog(INFO, "     ------> LPCE ExecInitMaterial###################inner %ld  outer %ld", matstate->ss.ps.plan->inner_relids, matstate->ss.ps.plan->outer_relids);
    ...
    int node_id = matstate->ss.ps.plan->plan_node_id;
    card_runLog[node_id].estimate_card = matstate->ss.ps.plan->plan_rows;
    node_style[node_id]              = 10; //nested loop join
    ...
    //card_runLog[node_id].inner_relids =  matstate->ss.ps.plan->inner_relids;
    card_runLog[node_id].outer_relids =  matstate->ss.ps.plan->outer_relids;
...
ExecEndMaterial(MaterialState *node)
{
    ...
    elog(INFO,"     ------> ------> LPCE ExecEndMaterial");
	/*
