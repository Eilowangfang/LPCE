		...
		switch (node->hj_JoinState)
		{
			...
			case HJ_BUILD_HASHTABLE:
				...
				if (parallel)
				{
                    ...
                    elog(INFO, "     ------>------>------> LPCE ExecHashJoinImpl Parallel build_barrier");
                ...
                elog(INFO, "     ------>------>------> LPCE ExecHashJoinImpl HJ_BUILD_HASHTABLE DONE");
                //elog(INFO, "     ------>------> LPCE ExecHashJoinImpl hash node end at total time: %.3f ", innerPlanState(node)->instrument->total);
                //elog(INFO, "     ------>------> LPCE ExecHashJoinImpl hash node id %ld and outer node id %ld", innerPlanState(node)->plan->plan_node_id, outerPlanState(node)->plan->plan_node_id);
                //elog(INFO, "     ------>------> LPCE ExecHashJoinImpl hash node estimate %.0f and outer node estimate %.0f", innerPlanState(node)->plan->plan_rows, outerPlanState(node)->plan->plan_rows);

                //LPCE
                if (innerPlanState(node)->instrument){

                    elog(INFO, "     ------>------> LPCE ExecHashJoinImpl hash node real %.0f and outer node real %.0f", innerPlanState(node)->instrument->ntuples, outerPlanState(node)->instrument->ntuples);
                    ...
                    * */
                    if(LPCECheck && !LPCERerun){
                        if(hashNode_actual_tuples < 1.0)
                            hashNode_actual_tuples = 1.0;
                        if(card_runLog[hash_node_id].estimate_card < 1.0)
                            card_runLog[hash_node_id].estimate_card = 1.0;

                        if(now_hashjoin_id >= 4 &&
                            ((hashNode_actual_tuples/card_runLog[hash_node_id].estimate_card) > 50 || (card_runLog[hash_node_id].estimate_card/hashNode_actual_tuples) > 50)){
                            LPCERerun    = true;
                            elog(INFO, "     +++++++>++++++> LPCE ExecHashJoinImpl Trigger Re-optimization Point: %d ", innerPlanState(node)->plan->plan_node_id);
			...
			case HJ_NEED_NEW_OUTER:
                ...
                //elog(INFO, "     ------>------>------> LPCE ExecHashJoinImpl HJ_NEED_NEW_OUTER");
				/* FALL THRU */
...
ExecInitHashJoin(HashJoin *node, EState *estate, int eflags)
{
    ...
    elog(INFO, "     ------> LPCE ExecInitHashJoin at node %ld estimate tuples %.0f", hjstate->js.ps.plan->plan_node_id, hjstate->js.ps.plan->plan_rows);
    //elog(INFO, "     ------> LPCE ExecInitHashJoin###################inner %ld  outer %ld", hjstate->js.ps.plan->inner_relids, hjstate->js.ps.plan->outer_relids);
    //elog(INFO, "     ------> LPCE ExecInitHashJoin estate %s", estate->es_sourceText);
	...
	//elog(INFO, "     ------> LPCE ExecInitHashJoin at inner node id %ld outer node id %ld", innerPlanState(hjstate)->plan->plan_node_id, outerPlanState(hjstate)->plan->plan_node_id);
...
{
    elog(INFO, "     ------>------> LPCE ExecEndHashJoin ");
