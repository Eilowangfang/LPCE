...
tts_minimal_store_tuple(TupleTableSlot *slot, MinimalTuple mtup, bool shouldFree)
{
    ...
    //elog(INFO, "     @@@@@@@@@@@ LPCE AAAAAAAAAAAA %ld %ld %s", mtup->t_len, MINIMAL_TUPLE_OFFSET, mslot->minhdr.t_data);
//	if(testPurpose)
//        elog(INFO, "     ------> LPCE AAAAAAAAAAAA %ld %ld %s", mtup->t_len, MINIMAL_TUPLE_OFFSET, (char *) mtup - MINIMAL_TUPLE_OFFSET);
//    if(testPurpose) {
//        elog(INFO, "     ------> LPCE BBBBBBBBBBB %ld %ld ||   %s ", slot->tts_flags, mslot->minhdr.t_data->t_infomask2,
...
slot_getsomeattrs_int(TupleTableSlot *slot, int attnum)
{
	...
	if (unlikely(attnum > slot->tts_tupleDescriptor->natts)) {
        elog(ERROR, "LPCE xxxxxxxxxxxxxxxxxxx %d", attnum);
