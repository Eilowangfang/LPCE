		   ...
		   bool partial_path)
{
    ...
    //elog(INFO, "     ------>------>------> LPCE final_cost_index %.0f cost %lf %lf", path->path.rows, startup_cost, startup_cost + run_cost);
...
//            int outer_node_id = card_runLog[i].outer_node_id;
//
//            if(card_runLog[inner_node_id].done_work && card_runLog[i].inner_relids == path->parent->relids->words[0]){
//                startup_cost -= input_startup_cost;
//                run_cost = input_total_cost - input_startup_cost;
//            }
//
//            if(card_runLog[outer_node_id].done_work && card_runLog[i].outer_relids == path->parent->relids->words[0]){
//                startup_cost -= input_startup_cost;
//                run_cost = input_total_cost - input_startup_cost;
//            }
//        }
...
//    if(outer_path->parent->relids->words[0] == 146 || outer_path->parent->relids->words[0] == 32 ||
//       inner_path->parent->relids->words[0] == 146 || inner_path->parent->relids->words[0] == 32)
//        elog(INFO, "     LPCE inital_cost_nestjoin =============== 111000000000000000000000 %ld %ld %lf %lf %lf %lf", outer_path->parent->relids->words[0], inner_path->parent->relids->words[0],
//             inner_path->total_cost, inner_path->startup_cost,  inner_rescan_total_cost, inner_rescan_start_cost);
     ...
     * During the re-optimization, the cost of outer path can be regarded
     * as 0
     * */
    if(LPCECheck && LPCERerun){
        for(int i = 0; i < NODE_NUM_MAX; i++){
            ...
            if(card_runLog[i].inner_relids == inner_path->parent->relids->words[0]){
                if(card_runLog[inner_node_id].done_work){
                    ...
                    run_cost -= inner_run_cost;
                    //elog(INFO, "     LPCE initial_cost_nestloop ########## %ld", card_runLog[i].inner_relids);
                }
            }
            ...
            if(card_runLog[i].outer_relids == inner_path->parent->relids->words[0]){
                if(card_runLog[outer_node_id].done_work){
                    startup_cost -= inner_path->startup_cost;
                    run_cost -= inner_run_cost;
                    //elog(INFO, "     LPCE initial_cost_nestloop ^^^^^^^^^^ %ld", card_runLog[i].outer_relids);
                }
            }
					   ...
					   JoinPathExtraData *extra)
{
    ...
    //elog(INFO, "     LPCE initial_cost_mergejoin inner ############################## %ld", inner_path->parent->relids->words[0]);
    //elog(INFO, "     LPCE initial_cost_mergejoin outer ############################## %ld", outer_path->parent->relids->words[0]);
    ...
    if(LPCECheck && LPCERerun){
        for(int i = 0; i < NODE_NUM_MAX; i++){
            ...
            if(card_runLog[i].inner_relids == inner_path->parent->relids->words[0] && node_style[inner_node_id] == 6){
                if(card_runLog[inner_node_id].done_work){
                    skip_inner_sort_reopt = true;
                    elog(INFO, "     LPCE initial_cost_mergejoin ^###################### %ld", card_runLog[i].inner_relids);
                }
            }

            if(card_runLog[i].outer_relids == outer_path->parent->relids->words[0] && node_style[outer_node_id] == 6){
                if(card_runLog[outer_node_id].done_work){
                    skip_outer_sort_reopt = true;
                    elog(INFO, "     LPCE initial_cost_mergejoin ^###########^^^^^^^^^ %ld", card_runLog[i].outer_relids);
                }
            }
        }
    }
					 ...
					 JoinPathExtraData *extra)
{
    ...
    //elog(INFO, "     ------>------>------> LPCE final_cost_mergejoin  %lf and %lf and %lf ", mergejointuples, inner_rows, outer_rows);
					  ...
					  bool parallel_hash)
{
	...
	//LPCE
	Cost        accum_inner_startup_cost = 0;
    Cost        accum_outer_startup_cost = 0;
    Cost        accum_inner_hash_cost = 0;
    Cost        accum_inner_run_cost = 0;
    Cost        accum_outer_run_cost = 0;
	...
	//LPCE
    accum_inner_startup_cost += inner_path->total_cost;
    accum_outer_startup_cost += outer_path->startup_cost;
    accum_outer_run_cost  += outer_path->total_cost - outer_path->startup_cost;
    ...
    //LPCE
    accum_inner_startup_cost += (cpu_operator_cost * num_hashclauses + cpu_tuple_cost)
                                * inner_path_rows;
    accum_outer_run_cost += cpu_operator_cost * num_hashclauses * outer_path_rows;
	...
	if (numbatches > 1)
	{
        ...
        accum_inner_startup_cost += seq_page_cost * innerpages;
        accum_inner_run_cost += seq_page_cost * (innerpages + 2 * outerpages);
	}
    ...
    if(LPCECheck && LPCERerun){
        ...
        for(int i = 0; i < NODE_NUM_MAX; i++){
            ...
            if(node_style[i] == 7
               && card_runLog[i].inner_relids == inner_path->parent->relids->words[0]){
                ...
                if (card_runLog[hash_node_id].done_work){
                    ...
                    startup_cost -= accum_inner_startup_cost;
                    run_cost -= accum_inner_run_cost;
                }
                continue;
            }
            ...
            if(card_runLog[i].outer_relids == inner_path->parent->relids->words[0]){
                int inner_node_id = card_runLog[i].outer_node_id;
                if (card_runLog[inner_node_id].done_work){
                    //if its done work, set the hash node cost as 0
                    //inner_path->total_cost is the cost of hash node
                    startup_cost -= inner_path->total_cost;
            ...
            if(card_runLog[i].inner_relids == outer_path->parent->relids->words[0]){
                int outer_node_id = card_runLog[i].inner_node_id;
                if (card_runLog[outer_node_id].done_work){
                    //if its done work, set the hash node cost as 0
                    //inner_path->total_cost is the cost of hash node
                    startup_cost -= accum_outer_startup_cost;
                    run_cost -= accum_outer_run_cost;
                }
            }
            if(card_runLog[i].outer_relids == outer_path->parent->relids->words[0]){
                int outer_node_id = card_runLog[i].outer_node_id;
					...
					JoinPathExtraData *extra)
{
	...
	if (path->jpath.path.parallel_workers > 0)
	{
		...
		path->jpath.path.rows =
			clamp_row_est(path->jpath.path.rows / parallel_divisor);
        //elog(INFO, "LPCE Call path->jpath.path.rows %lf || %lf", path->jpath.path.rows, parallel_divisor);
	}
    ...
    //elog(INFO, "     ------>------>------> LPCE final_cost_hashjoin %.0f and %.0f", inner_path_rows, outer_path_rows);
...
/*    if(LPCECheck && LPCERerun){
        ...
        for(int i = 0; i < NODE_NUM_MAX; i++){
            //elog(INFO, "     ------>------>------> LPCE node id %ld and  %ld  ", i, card_runLog[i].done_work);
            ...
            if(node_style[i] == 7
               ...
               && card_runLog[i].outer_relids == outer_path->parent->relids->words[0]){
                ...
                int outer_node_id = card_runLog[i].outer_node_id;
                if (card_runLog[outer_node_id].done_work){
                    //if its done work, set the outer node cost as 0
                    //outer_path->total_cost is the cost of hash node
    ...
    //elog(INFO, "     ------> LPCE final_cost_hashjoin: %lf  %lf", startup_cost, startup_cost + run_cost);
	path->jpath.path.startup_cost = startup_cost;
    path->jpath.path.total_cost = startup_cost + run_cost;
}
...
set_baserel_rows_estimate_standard(PlannerInfo *root, RelOptInfo *rel)
{
    ...
    elog(INFO, "LPCE base relation size: %lf", nrows);
						   ...
						   List *restrictlist)
{
	if (set_joinrel_size_estimates_hook) {
        (*set_joinrel_size_estimates_hook)(root, rel,
                                           ...
                                           sjinfo,
                                           restrictlist);
        //elog(INFO, "    LPCE call set_joinrel_size_estimates_hook");
    }
	else
						   ...
						   List *restrictlist_in)
{
	...
	switch (jointype)
	{
		case JOIN_INNER:
            ...
            //elog(INFO, "LPCE join size estimate: %lf = %lf %lf %lf %lf", nrows, outer_rows, inner_rows, fkselec, jselec);
			/* pselec not used */
			break;
		case JOIN_LEFT:
			nrows = outer_rows * inner_rows * fkselec * jselec;
    ...
    //elog(INFO, "LPCE join size: %lf", clamp_row_est(nrows));
