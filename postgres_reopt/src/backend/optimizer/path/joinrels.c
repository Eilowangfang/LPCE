...
join_search_one_level(PlannerInfo *root, int level)
{
    ...
    //elog(INFO, "*****************LPCE start left-sided plan join cost %d", level);
	/*
	...
	foreach(r, joinrels[level - 1])
	{
			...
			has_join_restriction(root, old_rel))
		{
			...
			else				/* consider all initial rels */
			{
                ...
                //elog(INFO, "        LPCE consider left-side plan with right relation as %s", nodeToString((Node *) other_rels_list));
			}
	...
	for (k = 2;; k++)
	{
        elog(INFO, "      LPCE start the bushy plan join cost %d", level);
							...
							SpecialJoinInfo *sjinfo, List *restrictlist)
{
	...
	switch (sjinfo->jointype)
	{
		case JOIN_INNER:
            ...
            //elog(INFO, " ------> LPCE populate_joinrel_with_paths rel1 join rel2");
			add_paths_to_joinrel(root, joinrel, rel1, rel2,
								 JOIN_INNER, sjinfo,
								 restrictlist);
            //elog(INFO, " ------> LPCE populate_joinrel_with_paths rel2 join rel1");
