...
make_one_rel(PlannerInfo *root, List *joinlist)
{
    ...
    elog(INFO, "     ------>------> LPCE make_one_rel make_one_rel");
	/*
...
standard_join_search(PlannerInfo *root, int levels_needed, List *initial_rels)
{
	...
	{
		ListCell   *lc;
        elog(INFO, "LPCE all possible pairs of relations to be joined at level %d ", lev);
		/*
		...
		foreach(lc, root->join_rel_level[lev])
		{
			...
			if (lev < levels_needed)
                ...
                //elog(INFO, "LPCE find and save cheapest paths for this rel of level %d ", lev);
