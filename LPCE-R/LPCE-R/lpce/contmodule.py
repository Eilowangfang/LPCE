...
class CONTENTmodule(torch.nn.Module):
    ...
    def _base_call_postgre(self, operation_feat, filter_feat, join_feat, card_feat):
        device = next(self.parameters()).device
        ...
        card_feat = F.relu(self.feature_mpl_card(card_feat))
        card_feat = F.relu(self.feature_mpl_card_2(card_feat))
        x = torch.cat((operation_feat, filter_feat, join_feat, card_feat), 1)
    ...
    def _join_call_postgre(self, operation_feat, filter_feat, join_feat, card_feat, left_child, right_child):
        operation_feat = F.relu(self.feature_mpl_operation(operation_feat))
        operation_feat = F.relu(self.feature_mpl_operation_2(operation_feat))
        filter_feat = F.relu(self.feature_mpl_filter(filter_feat))
        filter_feat = F.relu(self.feature_mpl_filter_2(filter_feat))
